...
impl<const COUNT: usize, S: Slot<COUNT> + Default> TrieEntry<COUNT, S> {
    ...
    pub fn index(this: EntryPtr<COUNT, S>, idx: SlotId<COUNT>) -> impl Ptr<S> {
        this.map(move |entry| unsafe { entry.slots.get_unchecked(idx.0) })
    }

    pub fn get(this: EntryPtr<COUNT, S>, id: u32) -> Result<Option<impl Ptr<S>>, S::Err> {
        let id = usize::try_from(id).unwrap();
        Self::get_inner(this, id)
    }

    #[tailcall]
    fn get_inner(this: EntryPtr<COUNT, S>, id: usize) -> Result<Option<impl Ptr<S>>, S::Err> {
...
pub type EntryPtr<const COUNT: usize, S> = <S as Slot<COUNT>>::Ptr<TrieEntry<COUNT, S>>;
pub trait Slot<const COUNT: usize>: Sized {
    type Err;
    type Ptr<T>: Ptr<T>;
    fn child(&self) -> Result<Option<EntryPtr<COUNT, Self>>, Self::Err>;
}
...
mod tests {
    ...
    impl<const COUNT: usize> Slot<COUNT> for MySlot<COUNT> {
        ...
        fn child(&self) -> Result<Option<Self::Ptr<TrieEntry<COUNT, Self>>>, Self::Err> {
            Ok(self.child.borrow().clone())
        }
    }

    impl<const COUNT: usize> MySlot<COUNT> {
        fn set_child(&self, child: Option<EntryPtr<COUNT, Self>>) -> Option<EntryPtr<COUNT, Self>> {
