...
pub enum ElfError {
    #[display("ELF Executable is ill-formatted")]
    BadElf,
    #[display("ELF program header offsets would overflow or overflow the buffer")]
    InvalidOffsets,
    #[display("The program image isn't aligned to a 16 byte boundary")]
    NotAligned,
}
...
pub enum SegmentLoadError<E> {
    LoaderError(E),
    #[display("Invalid file range in segment")]
    InvalidFileRange,
    #[display("Invalid virtual memory range in segment")]
    InvalidVirtualRange,
    #[display("Source segment can't fit on the virtual range")]
    SourceLargerThanVirtualRange,
    #[display("Segment ranges would overflow")]
    RangeOverflow,
}
...
pub enum LoaderError<E> {
    LoaderError(E),
    #[display("Source can't fit in the virtual range")]
    SourceLargerThanVirtualRange,
}
